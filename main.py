...
@register("astrbot_plugin_xiuxian", "xiuxian-dev", "修仙文字RPG游戏，支持聊天指令和网页界面", "0.8.3")
class XiuxianPlugin(Star):
    ...
    def _get_afk_max_minutes(self) -> int:
        """读取并规整挂机修炼上限。"""
        max_minutes = self._get_cfg_int("afk_cultivate_max_minutes", 60)
        return max(1, max_minutes)

    def _build_cmd_help(self) -> list[tuple[str, str]]:
        """按当前配置动态生成帮助文案。"""
        afk_max_minutes = self._get_afk_max_minutes()
        return [
            ("帮助", "显示所有可用指令"),
            ("签到", "每日签到领取奖励"),
            ("面板", "查看角色属性面板"),
            ("修炼", "修炼获取经验"),
            ("挂机 <分钟>", f"挂机修炼(1~{afk_max_minutes}分钟)"),
            ("结算", "领取挂机修炼收益"),
            ("取消挂机", "随时取消挂机并放弃本次收益"),
            ("历练", "进入秘境副本历练"),
            ("突破", "尝试突破当前境界"),
        ...
        if enable_web:
            try:
                ...
                self._web_server = WebServer(
                    ...
                    afk_cultivate_max_minutes=self._get_afk_max_minutes(),
                )
    ...
    async def show_help(self, event: AstrMessageEvent):
        """显示所有可用指令。"""
        img_bytes = renderer.render_help(self._build_cmd_help(), CMD_PREFIX)
