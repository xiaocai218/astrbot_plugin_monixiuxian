...
def generate_daily_items(target_date: date | None = None) -> list[dict]:
    ...
    while len(items) < SHOP_ITEM_COUNT and attempts < max_attempts:
        ...
        if cat == "consumable":
            ...
            # 避免在函数作用域内重新绑定 PILL_GRADE_NAMES，
            # 否则前面的丹药分支会被 Python 判定为读取未初始化的局部变量。
            from .pills import PILL_REGISTRY as _PILL_REGISTRY, PILL_TIER_NAMES as _PILL_TIER_NAMES
