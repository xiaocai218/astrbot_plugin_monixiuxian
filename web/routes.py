...
def create_router(
    ...
    try:
        normalized_afk_max_minutes = int(afk_cultivate_max_minutes)
    except (TypeError, ValueError):
        normalized_afk_max_minutes = 60
    normalized_afk_max_minutes = max(1, normalized_afk_max_minutes)
    ...
    async def index(request: Request):
        ...
        bootstrap = (
            ...
            f"window.__XIUXIAN_CONFIG__ = {json.dumps({'afkMaxMinutes': normalized_afk_max_minutes}, ensure_ascii=False)};"
            "</script>"
        )
    ...
    async def get_bind_key(request: Request):
        ...
        return {
            "success": True,
            "bind_key": key,
            "message": f"请在聊天平台中发送：{cmd_login} {key}",
            "expires_in": "7天",
        }
